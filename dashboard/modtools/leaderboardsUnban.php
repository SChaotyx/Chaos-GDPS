	...
	if ($pass == 1) {
		...
		if($gs->checkPermission($accountID, "toolLeaderboardsban")){
			if(!is_numeric($userID)){
				exit($dl->printBox("<h1>".$dl->getLocalizedString("unbanuser").'</h1>
	                       <p>Invalid UserID <a href="modtools/leaderboardsUnban.php">Try again</a></p>'));
			}
			$query = $db->prepare("UPDATE users SET isBanned = 0 WHERE userID = :id");
			$query->execute([':id' => $userID]);
			if($query->rowCount() != 0){
				//echo "Banned succesfully.";
				$dl->printBox("<h1>".$dl->getLocalizedString("unbanuser").'</h1>
	                       <p>Unban succesfully. <a href="">Please click here to continue.</a></p>');
			}else{
				//echo "Ban failed.";
			$dl->printBox("<h1>".$dl->getLocalizedString("unbanuser").'</h1>
	                       <p>Unban failed <a href="modtools/leaderboardsUnban.php">Try again</a></p>');
			}
			...
			exit($dl->printBox("<h1>".$dl->getLocalizedString("unbanuser").'</h1>
	                       <p>You do not have the permission to do this action. <a href="modtools/leaderboardsUnban.php">Try again</a></p>'));
		}
	}else{
		//echo "Invalid password or nonexistant account. <a href='leaderboardsBan.php'>Try again</a>";
		$dl->printBox("<h1>".$dl->getLocalizedString("unbanuser").'</h1>
	                       <p>Invalid password or nonexistant account. <a href="modtools/leaderboardsUnban.php">Try again</a></p>');
	}
}else{
		$dl->printBox('<h1>'.$dl->getLocalizedString("unbanuser").'</h1>
				<form action="" method="post">
					...
					<button type="submit" class="btn btn-primary btn-block">'.$dl->getLocalizedString("unbanuser").'</button>
				</form>',"mod");
}
?>